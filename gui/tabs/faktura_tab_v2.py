...
class FakturaTabV2(QWidget):
    ...
    # Signal emitted when data changes (for dirty tracking)
    data_changed = Signal()
    ...
    def _on_import_finished(self, result):
        ...
        try:
            ...
            if using_assembly:
                ...
                logger.debug(f"🔍 [dialog check] has_origin_statement={has_origin_statement}, items={len(items)}")
                if has_origin_statement:
                    # ✅ Faktura IMA izjavu → PE2 dialog
                    logger.debug(f"🔍 [dialog check] → otvaram PE2 dialog")
                    self._show_pe2_dialog(invoice_name)
                elif self._should_show_eur1_dialog(items):
                    # ❌ Faktura NEMA izjavu → EUR.1 dialog
                    logger.debug(f"🔍 [dialog check] → otvaram EUR.1 dialog")
                    self._show_eur1_dialog()
                else:
                    logger.debug(f"🔍 [dialog check] → nema dijaloga")
