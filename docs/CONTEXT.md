...
> Sadrži SAMO evergreen, cross-cutting pravila koja nisu vidljiva iz samog koda
> (sekcije 1-14 ispod). Dated hronologija pojedinačnih sesija je u
> `docs/context/history.md` — taj fajl se NE čita cijeli, samo pretražuje
> (vidi §13 i posljednju sekciju ovog fajla).
> Posljednje ažuriranje: 2026-07-30
...
Ovaj fajl (docs/CONTEXT.md) i `docs/context/history.md` imaju različitu ulogu —
ne dodavati novi sadržaj na pogrešno mjesto:

- **docs/CONTEXT.md (ovaj fajl, sekcije 1-14)** — samo evergreen pravila koja
  važe unaprijed, bez datuma vezanog za konkretnu sesiju. Dodati ovdje SAMO
  kad postojeće numerisano poglavlje (1-12, 14) dobije trajno pravilo koje
  agent mora znati PRIJE kodiranja svaki put. Max 5-6 linija po dodatku.
- **docs/context/history.md** — dated, append-only log pojedinačnih odluka,
  bugova i sesija. Ovdje ide sve što je vezano za konkretan datum/fazu/task,
  čak i ako je bitno — to je normalan slučaj, ne izuzetak.

Claude ažurira odgovarajući fajl na kraju svake sesije gdje je:
...
## Hronologija sesija — docs/context/history.md

Detaljan, dated log 88 pojedinačnih odluka/bugova/faza (2026-05 — 2026-07,
bivše sekcije 15-102 ovog fajla) premješten je u `docs/context/history.md`
2026-07-30 zbog veličine (fajl je narastao na ~440k tokena pri punom
čitanju, a ovaj CONTEXT.md se čita OBAVEZNO na početku svake sesije za
svakog agenta — vidi AGENTS.md "Kontekst projekta").

**Ne čitati taj fajl u cijelosti.** Pretražiti (Grep) po ključnoj riječi,
temi ili datumu prije nego što se donese zaključak da nešto nije
