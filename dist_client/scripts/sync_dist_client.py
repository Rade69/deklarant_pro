...
def normalize(content: bytes) -> bytes:
    """Normalizuj BOM, trailing whitespace i CRLF/LF stil.

    POPRAVKA (2026-07-27): bez CRLF->LF normalizacije, svaki fajl gdje root
    koristi LF (npr. sve fajlove koje Claude Code alati pišu) a dist_client
    kopija CRLF (Path.write_text na Windowsu) je lažno prijavljen kao
    "STVARNA RAZLIKA" iako je sadržaj identičan (potvrđeno
    diff --strip-trailing-cr). Vidi
    agent_reports/2026-07-27_popravka-wiring-gapova-agent-v2.md.
    """
    content = content.lstrip(b"\xef\xbb\xbf").rstrip()
